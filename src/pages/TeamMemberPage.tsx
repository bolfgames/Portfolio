...
import ResponsiveSlideshow from '../components/portfolio/ResponsiveSlideshow';
import TweenMaxSlideshow from '../components/portfolio/TweenMaxSlideshow';
...
function TeamMemberPage() {
  ...
  const isAtaCem = member?.name.toLowerCase().includes('ata');
  const isErdem = member?.name.toLowerCase().includes('erdem');
  // Emir Sayın için yeşil, Ahmet Emir için mor (designer), Ata Cem ve Erdem için altın, diğerleri için role color
  const accentColor = isEmir ? '#2ECC71' : (isAhmetEmir ? '#9B59B6' : (isAtaCem ? '#FFD700' : (isErdem ? '#FFD700' : roleColor)));
